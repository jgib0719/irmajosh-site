...
export function openScheduleModal(sourceId, sourceType, title, date, activeTab){
  ...
  if (workerWorks) populateTimeSlots(7,19);
  else populateTimeSlots(0,24);
  ...
  const pickedDate = new Date((new Date(date)).toDateString());
    ...
    if (pickedDate.getTime() === today.getTime()){
      // choose nearest 15-min slot >= now (bounded to shift)
      const now = new Date();
      const hh = now.getHours();
      const mm = now.getMinutes();
      const round = Math.ceil(mm/15)*15;
      let candidateH = hh;
...
export function populateTimeSlots(startH=7, endH=19, step=15){
